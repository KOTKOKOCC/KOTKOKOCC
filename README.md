# Hi there 👋 I'm KOTKOKOCC!
<a href="https://python.org"><img src="https://img.shields.io/badge/-Python-3776AB?style=flat&logo=python&logoColor=white"/></a>
<a href="https://mikrotik.com"><img src="https://img.shields.io/badge/-Mikrotik-293239?style=flat&logo=mikrotik&logoColor=white"/></a>
<a href="https://eltex-co.ru"><img src="https://shields.io" /></a>
<a href="https://linux.org"><img src="https://img.shields.io/badge/-Linux-FCC624?style=flat&logo=linux&logoColor=white"/></a>

### 🛠 Сетевой инженер & Автоматизатор (Network Engineer)

Я занимаюсь проектированием и поддержкой сетевой инфраструктуры. Мой фокус — автоматизация рутины, чтобы сетевые инженеры могли заниматься творчеством, а не копипастом конфигов.

---

#### 🔭 Мои проекты (Current Roadmap)


| Проект | Статус | Описание |
| :--- | :---: | :--- |
| **[Eltex_change_pass](https://github.com)** | 🚀 **Stable Release** | Автосмена паролей на Eltex-MES через Python. |
| **Hotspot Captive Portal** | 🧪 `Testing` | Backend для MikroTik: авторизация + аналитика. |
| **Network Mapper** | 🗺 `Concept` | Визуализация топологии сети в реальном времени. |

> **Note:** Многие проекты пока в приватных репозиториях. Готовлю их к Open Source релизу! 🚀

---

---
#### 💰 Поддержка (Sponsorship)
Ваша поддержка помогает мне выделять больше времени на Open Source и оплачивать облачные ресурсы для тяжелых сетевых лабораторий.


| Метод | Детали |
| :--- | :--- |
| **Bybit Card** | [Получить бонус при регистрации 🎁](https://www.bybit.com/invite?ref=LVWJZR3) |
| **Карта МИР (Сбер)** | `2202 2080 9806 3523` |
| **USDT (BEP20)** | `0x84eeb811d0121f7f19a7f5e09f5e9ea7ab480157` |
| **ETH (ERC20)** | `0x84eeb811d0121f7f19a7f5e09f5e9ea7ab480157` |
| **USD** <img src="https://img.shields.io/badge/-MasterCard-EB001B?style=flat&logo=mastercard&logoColor=white"/>| `5341 8376 0997 6164` |

> **Важно:** При отправке крипты убедитесь, что выбрали правильную сеть (**BEP20** для USDT или **ERC20** для ETH), чтобы средства не потерялись.

---



#### 📫 Связь со мной:
* **Telegram:** [@i_am_kotkokocc](https://t.me)

---
*С благодарностью за поддержку!* ⚡
